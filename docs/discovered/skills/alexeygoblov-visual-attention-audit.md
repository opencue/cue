---
title: "AlexeyGOblov/visual-attention-audit — Claude Code skill discovered by cue"
description: "Measure visual clutter and where the eye lands. Rosenholtz Feature Congestion and Subband Entropy on numpy/scipy/Pillow, plus ranking a design against its neigh"
layout: page
updated: 2026-09-09
tags: [claude-code, skill, creative-media]
---

# 💎 [AlexeyGOblov/visual-attention-audit](https://github.com/AlexeyGOblov/visual-attention-audit)

**★ 0** · exceptional (score 11) · Python · claude-skill, computer-vision, design-tools, feature-congestion, saliency

> Measure visual clutter and where the eye lands. Rosenholtz Feature Congestion and Subband Entropy on numpy/scipy/Pillow, plus ranking a design against its neighbours in a grid.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 11 based on:
- ✅ Contains SKILL.md
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-09-09
- 🏷️ Tags: claude-skill, computer-vision, design-tools, feature-congestion, saliency, ux-research, visual-attention, visual-clutter

## Best fit cue profiles

[creative-media](../creative-media.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add AlexeyGOblov/visual-attention-audit --profile creative-media
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/AlexeyGOblov/visual-attention-audit)

---
title: "linnea-bakshi/gha-doctor — Claude Code skill discovered by cue"
description: "Diagnose your GitHub Actions: flaky jobs, wasted minutes, slow steps, cache problems, and workflow anti-patterns — in one command, zero config."
layout: page
updated: 2026-09-09
tags: [claude-code, skill, core]
---

# ✨ [linnea-bakshi/gha-doctor](https://github.com/linnea-bakshi/gha-doctor)

**★ 2** · strong (score 6.5) · Go · actions, ci, ci-cd, cli, continuous-integration

> Diagnose your GitHub Actions: flaky jobs, wasted minutes, slow steps, cache problems, and workflow anti-patterns — in one command, zero config.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6.5 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-09-09
- 🏷️ Tags: actions, ci, ci-cd, cli, continuous-integration, developer-tools, devops, flaky-tests, github-actions, golang, linter, mcp, mcp-server, static-analysis

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add linnea-bakshi/gha-doctor --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/linnea-bakshi/gha-doctor)
